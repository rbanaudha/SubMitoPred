...
for($q=0;$q<=$#array;$q++)
{
	...
	for($x=7;$x<=$#dom;$x++)
	{
	    ...
	    {
		$a=100;
		open(UDOMAIN_NON_MITO,">>$dir/Non_mito_udomain") or die "$!";
		print UDOMAIN_NON_MITO "$dom[0]\t$dom[$x]\n";
		close UDOMAIN_NON_MITO;
	    }
		...
		while($submitofile=<SUBMITO_DOMAIN>)
		{
		    ...
		    {
			$c=100;
			open(UDOMAIN_IN,">>$dir/Inner_mem_udomain") or die "$!";
			print UDOMAIN_IN "$dom[0]\t$dom[$x]\n";
			close UDOMAIN_IN;
		    }
		    if(("$dom[$x]" eq "$submito_domain[0]")&&("$submito_domain[1]" eq "OUTER_MEM"))
		    {
			$d=100;
			open(UDOMAIN_OU,">>$dir/Outer_mem_udomain") or die "$!";
			print UDOMAIN_OU "$dom[0]\t$dom[$x]\n";
			close UDOMAIN_OU;
		    }
		    if(("$dom[$x]" eq "$submito_domain[0]")&&("$submito_domain[1]" eq "INTER_MEM_SPACE"))
		    {
			$e=100;
			open(UDOMAIN_INTER,">>$dir/Inter_mem_udomain") or die "$!";
			print UDOMAIN_INTER "$dom[0]\t$dom[$x]\n";
			close UDOMAIN_INTER;
		    }
		    if(("$dom[$x]" eq "$submito_domain[0]")&&("$submito_domain[1]" eq "MATRIX"))
		    {
			$f=100;
			open(UDOMAIN_MAT,">>$dir/Matrix_udomain") or die "$!";
			print UDOMAIN_MAT "$dom[0]\t$dom[$x]\n";
			close UDOMAIN_MAT;
		    }
		}
	...
	if(($dom[3] >= $dom[4])&&($dom[3] >= $dom[5])&&($dom[3] >= $dom[6]))
	{
	    ...
	    print SVM_RE "$dom[0]\tMITOCHONDRIAL_INNER_MEMBRANE\t$dom[3]\n";
	    close SVM_RE;
	}
	if(($dom[4] >= $dom[3])&&($dom[4] >= $dom[5])&&($dom[4] >= $dom[6]))
	{
	    open(SVM_RE,">>$dir/svm_result") or die "$!";
	    print SVM_RE "$dom[0]\tMITOCHONDRIAL_OUTER_MEMBRANE\t$dom[4]\n";
	    close SVM_RE;
	}
	if(($dom[5] >= $dom[3])&&($dom[5] >= $dom[4])&&($dom[5] >= $dom[6]))
	{
	    open(SVM_RE,">>$dir/svm_result") or die "$!";
	    print SVM_RE "$dom[0]\tMITOCHONDRIAL_INTER_MEMBRANE_SPACE\t$dom[5]\n";
	    close SVM_RE;
	}
	if(($dom[6] >= $dom[3])&&($dom[6] >= $dom[4])&&($dom[6] >= $dom[5]))
	{
	    open(SVM_RE,">>$dir/svm_result") or die "$!";
	    print SVM_RE "$dom[0]\tMITOCHONDRIAL_MATRIX\t$dom[6]\n";
	    close SVM_RE;
	}
    }
	...
	{
	    open(SVM_RE,">>$dir/svm_result") or die "$!";
	    print SVM_RE "$dom[0]\tNON_MITOCHONDRIA\t$dom[2]\n";
	    close SVM_RE;
	}
	if($dom[2] >= $svm_th)
	{
...
print "<table border='1' width='400' align='center'><tr><th>Protein ID</th><th>Prediction</th><th>Pfam Domain/SVM Score</th></tr>";
}
if($total_seq == 0)
{
    system "/bin/cat $dir/Pfam_result $dir/svm_result >$dir/Prediction";
